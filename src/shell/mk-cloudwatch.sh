...
# Last updated: 2026-03-26 20:41:04
################################################################################
set -euo pipefail
...
EOT
  ...
  # 1-4. ロググループタグ設定
EOT
  aws logs tag-log-group --log-group-name "$CW_LOG_GROUP_LAMBDA_NAME" --tags Project=${PRJ_PREFIX}-system,Component=lambda,${PRJ_TAG_KEY}=${PRJ_TAG_VALUE}
  aws logs tag-log-group --log-group-name "$CW_LOG_GROUP_EC2_NAME" --tags Project=${PRJ_PREFIX}-system,Component=ec2,${PRJ_TAG_KEY}=${PRJ_TAG_VALUE}
  aws logs tag-log-group --log-group-name "$CW_LOG_GROUP_SYSTEM_NAME" --tags Project=${PRJ_PREFIX}-system,Component=system,${PRJ_TAG_KEY}=${PRJ_TAG_VALUE}
