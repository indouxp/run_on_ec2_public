...
# TC-040.sh
#
# Subnetが存在する状態から実行、正常処理（削除して再作成）
#
...
# Subnetを新規作成（スクリプトが「Subnetあり」の状態で実行されるようにする）
PRE_SUBNET_ID=$(aws ec2 create-subnet \
  --vpc-id "${VPC_ID}" \
  --cidr-block "10.0.1.0/24" \
  --query "Subnet.SubnetId" --output text)
aws ec2 create-tags --resources "${PRE_SUBNET_ID}" --tags "Key=Name,Value=${SUBNET_NAME}"
echo "前処理: Subnet [${SUBNET_NAME}] (ID: ${PRE_SUBNET_ID}) を作成しました"
