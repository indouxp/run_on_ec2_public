...
if [[ -n "${ROLE_EXISTS}" ]]; then
  echo "前処理: ロール [${IAM_ROLE_BUILD_NAME}] を削除します"
  aws iam delete-role --role-name "${IAM_ROLE_BUILD_NAME}"
  echo "前処理: ロール [${IAM_ROLE_BUILD_NAME}] を削除しました"
else
  echo "前処理: ロール [${IAM_ROLE_BUILD_NAME}] は存在しません（削除不要）"
fi
